...
var g_Logger = new LoggerClass('microgateway-config kernel-utilities');
...
module.exports.loadStatus = (message, url, err, response, body, cb) => {
    ...
    if (url) {
        ...
        if ( failed ) {
            g_Logger.warn(message, 'download from', url,'returned', 
                            (response ? (response.statusCode + ' ' + response.statusMessage) : '', err ? err : ''));
        } else {
            g_Logger.info(message, 'download from', url, 'returned',
...
module.exports.validateUrls = (config) => {
    ...
    if (bootstrapUrl.hostname === 'apigee.net' ||
        ...
        publicKeyUrl.pathname.indexOf('...') > 0) {
            g_Logger.error('it looks like edge micro has not been configured, please see the admin guide');
