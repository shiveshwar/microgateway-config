...
function validateEdgeMicro(config) {
...
function validateQuota(config) {
  assert(config.quota.timeUnit === 'hour' ||
  config.quota.timeUnit === 'minute' ||
  config.quota.timeUnit === 'day' ||
  config.quota.timeUnit === 'week', 'invalid value for config.quota.timeUnit: ' + config.quota.timeUnit + ', valid values are hour, minute, day & week');
...
function validateQuotas(config) {
  ...
  Object.keys(config.quotas).forEach(timeUnit => {
      assert(timeUnit === 'default' ||
          ...
          timeUnit === 'minute' ||
          timeUnit === 'day' ||
          timeUnit === 'week', 'invalid value in config.quotas: ' + timeUnit + ', valid values are hour, minute, day, week, & default');
      //
...
function validateSpikeArrest(config) {
...
module.exports.validate = (config) => {
  ...
  if (!process.env.EDGEMICRO_LOCAL && !process.env.EDGEMICRO_LOCAL_PROXY) {
    validateEdgeConfig(config)
  }

  validateEdgeMicro(config)
