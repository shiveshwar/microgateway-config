...
function loadConfiguration(thisconfig, keys, callback) {
    ...
    _load(config, keys, function(err, proxies, products) {
        ...
        if (proxies && products) {
            ...
            const mergedConfig = kut.merge(config, _mapEdgeProxies(proxies), _mapEdgeProducts(config.environment,products));
...
const _load = function(config, keys, callback) {
    ...
    async.parallel([
    ...
    ], function(err, results) {   // finally use all result.
        ...
        try {
            ...
            if (proxyPattern) {
                ...
                for (counter = 0; counter < proxiesLen; counter ++) {
                    if (!kut.matchWildcard(apiProxies[counter].apiProxyName, proxyPattern)) {
            ...
            console.error('CRITICAL ERROR:', 'error parsing downloaded proxy list',
                err);
            return callback(err);
        }
        var proxies = proxyInfo && proxyInfo.apiProxies ? proxyInfo.apiProxies : [];
        if (!proxies) {
            console.warn('no edge micro proxies found in response');
        }
        if (proxies.length === 0) {
            console.warn( 'no edge micro proxies found in org');
        }
        ...
        try {
            ...
            if(err instanceof SyntaxError) console.error('CRITICAL ERROR:', 'error parsing downloaded product list',
                err);
            return callback(new Error('CRITICAL ERROR: error parsing downloaded product list'));
        }
        if (!productInfo) {
            console.warn( 'no edge micro products found in response');
        }
        ...
        if (products.length === 0) {
            console.warn('no products found in org');
        }
        ...
        if (results.length > 1 && results[2]) {
            ...
            console.warn('failed to download jwt_public_key');
        }
        //add jwk support
        try {
            ...
            console.warn('jwk keys are not enabled');
        }
        //add jwk support
...
const _mapEdgeProducts = function(env,products) {
    //const path_to_proxy = {};
    ...
    //
    var envProfs = products
    if ( env !== undefined && env ) {
        envProfs = products.filter(prod => { 
            return(prod.environments.includes(env))
        })    
    }
    //
    envProfs.forEach(function(product) {
