...
function networkLoadCallback(err, proxies, products, keys, config, callback) {
    ...
    if ( proxies && products ) {
        ...
        if ( mergedConfig ) {
            ...
            console.error('fatal:', 'cached config not available, unable to continue');
...
function loaderAddPublicKeySupport(config,results) {
    try {
        ...
        console.warn('jwt public keys are not enabled');
    }
}
...
function loaderUnloadProductInfo(results) {
    ...
    if ( results.length > 1 && results[1] ) {
        ...
        try {
            ...
            if ( err instanceof SyntaxError ) console.error('CRITICAL ERROR:', 'error parsing downloaded product list', err);
            throw err
        }
    }
    //
...
function loaderUnloadProxyInfo(results) {
    ...
    if ( results.length > 0 && results[0] ) {
        try {
            ...
            if ( proxyInfo.apiProxies && (typeof proxyInfo.apiProxies) && (proxyInfo.apiProxies.length) ) {
                //
            ...
            console.error('CRITICAL ERROR:', 'error parsing downloaded proxy list', err);
            throw err
        }
    }
    //
...
function loadJoinResults(err,results,config,keys,callback) {
    ...
    //
    if ( err ) {
        console.warn( 'error downloading config, please check bootstrap configuration', err);
        return callback(err)
    }
